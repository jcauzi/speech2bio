...
def _get_waveform_with_length(filename, max_duration, target_sample_rate):
    try:
        waveform, sample_rate = torchaudio.load(filename)
    except RuntimeError as e:
        import librosa
    ...
    # Calculate the original length in terms of samples
    original_length = waveform.size(1)

    # Clip or pad the waveform to the maximum duration
    max_samples = max_duration * target_sample_rate
    waveform = waveform[0, :max_samples]
    if waveform.shape[0] < max_samples:
        waveform = F.pad(waveform, (0, max_samples - waveform.shape[0]))

    # Compute the length after padding/clipping
    padded_length = min(original_length, max_samples)

    return waveform, padded_length
